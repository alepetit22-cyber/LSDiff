...
class TimeGradEstimator(PyTorchEstimator):
    def __init__(
        ...
        num_event_classes: int = 100,
        event_embed_dim: int = 8,
        cce_weight: float = 0.1,
        **kwargs,
    ) -> None:
    ...
    def create_training_network(self, device: torch.device) -> TimeGradTrainingNetwork:
        return TimeGradTrainingNetwork(
            ...
            num_event_classes=self.num_event_classes, # NOUVEAU
            event_embed_dim=self.event_embed_dim,     # NOUVEAU
            cce_weight=self.cce_weight,               # NOUVEAU
        ).to(device)

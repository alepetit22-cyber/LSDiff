...
class TimeGradTrainingNetwork(nn.Module):
    ...
    def __init__(
        ...
        num_event_classes: int = 100,                           # NOUVEAU
        event_embed_dim: int = 8,                               # NOUVEAU
        cce_weight: float = 0.1,                                # NOUVEAU
        ...
        **kwargs,
        ...
        self.continuous_dim = target_dim -1                     # Nouveau --> Retrait de la variable catégorielle
        self.num_event_classes = num_event_classes              # NOUVEAU
        self.event_embed_dim = event_embed_dim                  # NOUVEAU
        self.cce_weight = cce_weight                            # NOUVEAU
        ...
        # Nouveau --> Ajustement dynamique de la taille d'entrée du RNN pour inclure l'embedding
        adjusted_input_size = input_size - (len(lags_seq) * target_dim) + (len(lags_seq) * (self.continuous_dim + self.event_embed_dim))
        ...
        self.denoise_fn = EpsilonTheta(
            target_dim=self.continuous_dim,                     # NOUVEAU
        ...
        self.diffusion = GaussianDiffusion(
            self.denoise_fn,
            input_size=self.continuous_dim,                     # NOUVEAU
        ...
        # NOUVELLES COUCHES POUR LES EVENEMENTS
        self.event_embedding = nn.Embedding(num_embeddings=self.num_event_classes, embedding_dim=self.event_embed_dim)
        self.event_classifier = nn.Linear(num_cells, self.num_event_classes)
    ...
    def unroll(
        ...
        """
        ### NOUVEAU ###
        # Séparation des caractéristiques continues et de l'événement catégoriel
        lags_continuous = lags[:, :, :self.continuous_dim, :]
        lags_event = lags[:, :, self.continuous_dim, :].long()

        # Normalisation uniquement sur les variables continues
        lags_scaled = lags_continuous / scale.unsqueeze(-1)
        input_lags_continuous = lags_scaled.reshape(
            (-1, unroll_length, len(self.lags_seq) * self.continuous_dim)
        )

        # Embedding des événements
        lags_event_emb = self.event_embedding(lags_event) # shape: (B, seq_len, num_lags, embed_dim)
        input_lags_event = lags_event_emb.reshape(
            (-1, unroll_length, len(self.lags_seq) * self.event_embed_dim)
        )

        # Concaténation pour le RNN
        input_lags = torch.cat((input_lags_continuous, input_lags_event), dim=-1)
    ...
    def unroll_encoder(
        ...
        ### NOUVEAU ###
        # scale n'est appliqué qu'aux variables continues
        _, scale = self.scaler(
            past_target_cdf[:, -self.context_length :, :self.continuous_dim],
            past_observed_values[:, -self.context_length :, :self.continuous_dim],
        )
        ### ####### ###
    ...
    def forward(
        ...
        # Split continuous (7) et event (1)
        target_continuous = target[:, :, :self.continuous_dim]   # NOUVEAU
        target_event = target[:, :, self.continuous_dim].long()  # NOUVEAU
        ...
        # Loss de diffusion (Log-Likelihood) sur les continues
        likelihoods = self.diffusion.log_prob(target_continuous, distr_args).unsqueeze(-1)

        # assert_shape(likelihoods, (-1, seq_len, 1))

        # Ajout de la Loss par Cross-Entropy sur l'événement
        event_logits = self.event_classifier(rnn_outputs)
        loss_ce = F.cross_entropy(
            event_logits.reshape(-1, self.num_event_classes),
            target_event.reshape(-1),
            reduction='none'
        ).reshape(-1, seq_len, 1)

        # Combinaison des pertes
        total_loss_per_step = likelihoods + (self.cce_weight * loss_ce)
        ...
        loss_weights, _ = observed_values[:, :, :self.continuous_dim].min(dim=-1, keepdim=True) # NOUVEAU

        # assert_shape(loss_weights, (-1, seq_len, 1))

        loss = weighted_average(total_loss_per_step, weights=loss_weights, dim=1)

        # assert_shape(loss, (-1, -1, 1))

        # self.distribution = distr

        return (loss.mean(), total_loss_per_step, distr_args)


class TimeGradPredictionNetwork(TimeGradTrainingNetwork):
    @validated()
    def __init__(self, num_parallel_samples: int=50, **kwargs) -> None:
        super().__init__(**kwargs)
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            # Echantillonnage des variables continues (Diffusion)
            new_samples_continuous = self.diffusion.sample(cond=distr_args)

            # Echantillonnage de l'événement (Classification)
            event_logits = self.event_classifier(rnn_outputs)
            event_probs = F.softmax(event_logits, dim=-1).squeeze(1) # Probabilités
            new_samples_event = torch.multinomial(event_probs, 1).unsqueeze(-1).float() # Tirage de classe

            # Concaténation pour recréer un tenseur de dimension 8
            new_samples = torch.cat((new_samples_continuous, new_samples_event), dim=-1)

...
class Q0Dist(torch.nn.Module):
    ...
    def gp_regression(self, x: TensorType[float], prediction_length: int) -> MV:
        ...
        # repeat_factor peut être fractionnaire si la longueur n'est pas un multiple exact
        repeat_factor = math.ceil(x.shape[1] / self.freq)
        x_centered = x - loc_freq.repeat(1, repeat_factor)[:, :x.shape[1]]

        # Compute the regression mean.
        mean = x_centered @ self.K_inv_x_K_star
        repeat_pred = math.ceil(prediction_length / self.freq)
        mean = mean + loc_freq.repeat(1, repeat_pred)[:, :prediction_length]

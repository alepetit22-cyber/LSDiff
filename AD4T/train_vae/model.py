...
# All rights reserved.
# 
# Code is originally from the Tabsyn (https://arxiv.org/pdf/2310.09656) implementation
...
class VAE(nn.Module):
    def __init__(self, d_numerical, categories, num_layers, hid_dim, n_head = 1, factor = 4, bias = True, transformer=True):
        ...
        if transformer:
            ...
            self.decoder = MLP2(d_token,64,d_token)

    def get_embedding(self, x):
        return self.encoder_mu(x).detach() 
...
class Encoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True, transformer=True):
        ...
        if transformer:
            ...
            self.VAE_Encoder = MLP2(d_token,64,d_token)
...
class Decoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True, transformer=True):
        ...
        if transformer:
            ...
            self.VAE_Decoder = MLP2(d_token,64,d_token)
